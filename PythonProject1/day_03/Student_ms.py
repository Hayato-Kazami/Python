...
class StudentMS(object):
    ...
    def __init__(self):
        ...
        self.stu_dict = {}
        # 打开保存学生信息的文件。
        with open(StudentMS.stu_file_path,"r",encoding="utf-8") as fr:
            # 一次性读取文件中的所有行。
    ...
    def save(self):
        """
        保存学生信息到文件。
        当前内存中的 self.stu_dict 写回 stu.json。
        每个学生对象写成一行 JSON 字符串。
        """
        with open(StudentMS.stu_file_path,"w",encoding="utf-8") as fw:
    ...
    def show(self):
        ...
        print("【查询全部学生】")
        for stu in self.stu_dict.values():
            print(f"姓名：{stu.name},性别:{stu.gender},"
                  f"手机：{stu.tel},年龄：{stu.age},外号：{stu.nickname}")
    ...
    def query(self):
        ...
        print(f"查到学生，姓名：{stu.name}, 性别：{stu.gender}, "
              f"手机：{stu.tel}，年龄：{stu.age}，外号：{stu.nickname}")

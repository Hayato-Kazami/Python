"""BERT 模型 int8 动态量化（剧透版）。

对训练好的剧透 BERT 做后训练动态量化：只量化 nn.Linear（embedding / LayerNorm 保持 fp32），
...
QUANTIZED_PATH = conf.best_model_path.replace("best.pth", "quantized.pt")
...
def main():
    ...
    model = SpoilerBertClassifier()
    model.load_state_dict(torch.load(conf.best_model_path, map_location="cpu"))
    model.eval()
    ...
    size_before = os.path.getsize(conf.best_model_path)
